...
function renderAttributes<T extends keyof h.JSX.IntrinsicElements>(attribute: h.JSX.Properties<T>): string {
    return Object
        .entries(attribute)
        .map(([key, value]) => (
            typeof value === 'boolean'
                ? key
                : `${key}="${escapeHtml((value as number | string)?.toString()) ?? ''}"`
        ))
        .join(' ');
}
...
export function h<T extends keyof h.JSX.IntrinsicElements>(
    ...
    return EMPTY_ELEMENT_LIST.has(tagName)
        ? `<${tagName}${properties ? ` ${renderAttributes(properties)}` : ''}>`
        : `<${tagName}${properties ? ` ${renderAttributes(properties)}` : ''}>${children.flat().join('')}</${tagName}>`;
}
